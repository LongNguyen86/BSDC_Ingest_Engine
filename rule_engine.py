...
def clean_excel_text(text) -> str:
    ...
    # 1. Replace non-breaking spaces (\xa0) with standard space and strip whitespace
    s = str(text).replace("\xa0", " ").strip()

    # 2. Convert smart/curly quotes to standard ASCII quotes
    s = s.replace("“", '"').replace("”", '"').replace("’", "'").replace("‘", "'")

    # 3. Normalize Unicode characters (NFKC format)
...
def process_mapping_sheet(
    ...
    for idx, row in raw_df.iterrows():
        ...
        # Set default fallback indices if headers were not detected
        ...
        if any(kw in row_str.lower() for kw in ["table)", "(mb-", "(dp", "table", "section"]):
            ...
            if clean_sec_name and len(clean_sec_name) < 100:
                ...
                active_data_file = ""  # Reset active data file for new section
        ...
        # Forward fill active data file within section if current cell is empty
...
def find_mapping_file(ingest_dir: Path) -> Path:
    """Find the most recently modified mapping Excel file inside workspace/ingest/."""
    mapping_files = [
        f for f in ingest_dir.glob("*.xlsx")
        if not f.name.startswith("~$") and "mapping" in f.name.lower()
    ]
    ...
    print(f"🎯 Automatically found latest mapping file: {latest_file.name}")
    return latest_file
